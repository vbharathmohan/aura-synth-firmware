...
{
    if (speed_mmps <= MIN_SWIPE_SPEED_MMPS)
        return 0;
...
bool sensor_task_init(void)
{
    ...
    for (int i = 0; i < NUM_TOF_SENSORS; i++)
    {
        ...
        // note_event_post(instrument_to_slot(g_state.active_instrument),
        //                 150,            // Velocity (medium-loud)
        //                 SCALE_SPEED[i], // Pitch from C-major scale
        //                 false,          // loop
        //                 (int8_t)i,      // source
        //                 0);             // track
