...
int sampler_trigger(int slot_idx, uint8_t velocity, float speed, bool loop)
{
    ...
    float sample_rate_ratio = (slot->sample_rate > 0)
                                  ? (float)slot->sample_rate / (float)SAMPLE_RATE
                                  : 1.0f;

    s_voices[v].slot = slot;
    s_voices[v].position = 0.0f;
    s_voices[v].speed = speed * sample_rate_ratio;
    s_voices[v].volume = velocity / 255.0f;
    s_voices[v].active = true;
    s_voices[v].looping = loop;
    s_voices[v].paused = false;
...
bool sampler_pause_voice(int voice_idx)
{
    ...
    if (!s_voices[voice_idx].active || s_voices[voice_idx].paused)
    {
        return false;
    }
    s_voices[voice_idx].paused = true;
    return true;
}
...
bool sampler_resume_voice(int voice_idx)
{
    ...
    s_voices[voice_idx].paused = false;
    return true;
}
...
void sampler_render(audio_block_t *blk)
{
    for (int v = 0; v < SAMPLER_MAX_VOICES; v++)
    {
        ...
        if (!voice->active || voice->paused || !voice->slot)
            continue;
